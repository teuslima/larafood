...
class OrderService
{
    ...
    public function createNewOrder(array $order){
        ...
        $status = 'open';
        $tenant_id = $this->getTenantIdByOrder($order['token_company']);
        $comment = $order['comment'] ?? '';
        $client_id = $this->getClientIdByOrder();
        $table_id = $this->getTableIdByOrder($order['table'] ?? '');
    ...
    private function getIdentifyOrder($qtyCaraceters = 8)
    {
    ...
    {
        if($uuid){
            $table = $this->tableRepository->getTableByIdentify($uuid);
            return $table->id;
        }
